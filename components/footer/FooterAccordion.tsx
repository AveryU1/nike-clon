...
interface FooterState {
    onAccordion: boolean,
    setOnAccordion: (newValue: boolean) => void
}

const FooterAccordion = ({onAccordion, setOnAccordion}: FooterState) => {
    ...
    < >
        {
            footerAcordion.map((services) => (
                ...
                <div>
                    ...
                    {
                        services.fields.map(options => (
                        <li className='mb-[3px]'><Link href='/' className='text-[#7e7e7e] text-xs mb-3 font-medium hover:text-white' >{options.name}</Link></li>
                        ))
                    }
                    </ul>
                </div>
